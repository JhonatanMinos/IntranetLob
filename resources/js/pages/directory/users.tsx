...
import { Building2, Calendar, Mail, Pencil, Smartphone, Trash2 } from 'lucide-react';
...
import { destroy, index as users } from '@/routes/users';
...
export default function Users({ users, departments, stores, company }: UsersDirectoryProps) {
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="Usuarios" />
      <DirectoryLayout departments={departments} stores={stores} company={company}>
        <ScrollArea className="h-[90vh] md:h-[80vh] lg:h-[70vh]">
          {users.data.map((user) => (
            <Card key={user.id} className="mb-4 p-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {user.stores && user.stores.length > 0 ? (
                          user.stores.map((store) => (
                            ...
                            >
                              {store.city ? `${store.city} - ${store.name}` : store.name}
                            </Badge>
                          ))
                        ) : (
                          <span className="text-gray-500">Sin tiendas asignadas</span>
                        )}
                      </div>
                    </div>
                  </CardContent>
