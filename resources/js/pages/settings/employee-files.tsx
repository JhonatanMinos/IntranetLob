...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Expediente', href: edit().url }];

export default function EmployeeFiles() {
  const { auth, employeeFile } = usePage<SharedData & EmployeeFileProps>().props;
                  ...
                  <div>
                    ...
                    <CardDescription className="mt-0.5 flex flex-wrap items-center gap-2 text-sm font-medium">
                      <div className="flex items-center gap-1">
                        ...
                        Empleado: <span className="font-semibold">{auth.user.employeeNumber}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <BriefcaseBusiness className="h-4 w-4 text-muted-foreground" />
                        Puesto: <span className="font-semibold">{auth.user.position}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <CalendarRange className="h-4 w-4 text-muted-foreground" />
                        Ingreso: <span className="font-semibold">{auth.user.dateEntry}</span>
                      </div>
                    </CardDescription>
                  </div>
                </CardHeader>
            ...
            return (
              <div key={group.id}>
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold tracking-wider uppercase">{group.label}</span>
                </div>
                <Card>
                  <CardContent className="divide-y p-0">
                    {group.docs.map(({ key, label, description }) => (
                      <DocumentRow
                        key={key}
                        docKey={key}
                        label={label}
          ...
          <Card>
            <CardContent className="divide-y p-4">
              <form className="gap-2 space-y-6">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="emergency_contact_name">Telefono</Label>
                  <Input
