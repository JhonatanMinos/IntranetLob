...
export function CalendarEvent({ events }: CalendarAgendaProps) {
  ...
  const modifiers = {
    birthday: (date: Date) =>
      parsedEvents.some(
        (e) => e.type === 'birthday' && e.date.toDateString() === date.toDateString()
      ),

    holiday: (date: Date) =>
      parsedEvents.some(
        (e) => e.type === 'festivo' && e.date.toDateString() === date.toDateString()
      ),

    event: (date: Date) =>
      parsedEvents.some(
        (e) => e.type === 'evento' && e.date.toDateString() === date.toDateString()
      ),

    launch: (date: Date) =>
      parsedEvents.some(
        (e) => e.type === 'lanzamiento' && e.date.toDateString() === date.toDateString()
      ),
  };
