...
export function BirthDays({ birthdays }: Props) {
  ...
  return (
    <Card className="flex-1">
      ...
      <CardContent>
        {!birthdays?.length ? (
          <p className="py-6 text-center text-sm text-muted-foreground">Sin cumpleaños próximos</p>
        ) : (
                ...
                return (
                  <div key={id} className="flex items-center gap-3 rounded-lg bg-muted/40 p-3">
                    ...
                    <div className="ml-auto shrink-0 self-center">
                      <Badge variant={isbirthday ? 'default' : 'secondary'}>
                        {isbirthday
                          ? '🎂 Hoy'
                          : format(parseISO(birthday), 'dd MMM', { locale: es })}
                      </Badge>
                    </div>{' '}
                  </div>
                );
