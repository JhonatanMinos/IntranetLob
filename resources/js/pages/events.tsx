...
export default function Events({ results }: EventsProps) {
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="Eventos" />
      <div className="flex w-full flex-row items-center justify-between border-b border-white/10 px-6 py-4">
        <h2 className="text-xl font-bold tracking-wide">
          {format(today, 'MMMM', { locale: es }).toUpperCase()}
        </h2>
        <Button className="shrink-0 bg-blue-500 text-white" onClick={() => setOpenModal(true)}>
          <Plus size={16} /> agregar Evento
        </Button>
      </div>
      <div className="p-4">
        <EventsTable data={results} onEdit={handleEditOpen} onDelete={handleDelete} />
      </div>
